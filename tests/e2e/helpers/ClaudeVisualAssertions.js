...
class ClaudeVisualAssertions {
  constructor(config = {}) {
    ...
    if (!this.apiKey || !visualChecksEnabled) {
      if (!visualChecksEnabled) {
        console.log(
          'ℹ️  Visual assertions disabled (set ENABLE_VISUAL_CHECKS=true to enable)'
        );
      } else {
        console.warn(
          '⚠️  ANTHROPIC_API_KEY not set - visual assertions will be skipped'
        );
      }
      this.enabled = false;
      return;
    ...
    this.screenshotDir =
      ...
      path.join(__dirname, '../../../test-results/visual-assertions');
  ...
  async askClaude(question, includeScreenshot = true) {
    ...
    if (includeScreenshot) {
      const screenshot = await this.takeScreenshot(
        `query-${this.assertionResults.length + 1}`
      );
      content.push({
        type: 'image',
        source: {
  ...
  async visualAssert(description, expectedAnswer = 'yes') {
    ...
    const passed =
      answer.includes(expectedAnswer.toLowerCase()) || response === 'skipped';
  ...
  getResults() {
    ...
    const passed = this.assertionResults.filter(
