...
class ComputerUseTestRunner {
    constructor(config = {}) {
        ...
        this.screenshotDir = config.screenshotDir || path.join(__dirname, '../../test-results/computer-use');
    ...
    async findSimulator() {
        try {
            ...
            for (const runtime in devices.devices) {
                const bootedDevices = devices.devices[runtime].filter(d => d.state === 'Booted');
                if (bootedDevices.length > 0) {
                    this.deviceId = bootedDevices[0].udid;
                    console.log(`✅ Found booted simulator: ${bootedDevices[0].name} (${this.deviceId})`);
                    return;
                }
            }

            throw new Error('No booted simulator found. Please start the iOS Simulator.');
        } catch (error) {
    ...
    async takeScreenshot(name = 'screenshot') {
        ...
        try {
            await execAsync(`xcrun simctl io ${this.deviceId} screenshot "${filepath}"`);
    ...
    async sendMessage(instruction, includeScreenshot = true) {
        ...
        if (includeScreenshot) {
            const screenshot = await this.takeScreenshot(`step-${messages.length + 1}`);
            content.push({
                type: 'image',
                source: {
                    type: 'base64',
    ...
    async typeText(text) {
        try {
            ...
            await execAsync(`xcrun simctl io ${this.deviceId} sendText '${escapedText}'`);
