...
## Critical Refinements for Execution Readiness

### 1. Action Items Schema Compatibility & Migration

**Issue:** Phase 3 extends action items schema significantly, but existing implementation uses simpler schema.

**Solution:** Ensure backward compatibility with migration path:

```typescript
// Existing Phase 2 schema (maintained for compatibility)
...
// Phase 3 Extended schema (extends existing)
interface Phase3ActionItem extends ActionItem {
  // Phase 3 additions with defaults for backward compatibility
  propertyId?: string;
  organizationId?: string;
  autoGenerated?: boolean; // Default: false (manual)
  generatedBy?: 'marketing_agent' | 'property_management_agent' | 'user'; // Default: 'user'
  generatedAt?: Date; // Default: createdAt
  conversationStage?: 'prospect' | 'showing' | 'application' | 'move_in_prep' | 'move_in' | 'post_move_in' | 'maintenance' | 'renewal';
...
// Migration Strategy:
// 1. All existing action items remain valid (missing fields use defaults)
// 2. New action items use extended schema
// 3. Gradual migration: Update action items when accessed/modified
// 4. No breaking changes to existing UI components
```

**Migration Implementation:**
- Add migration utility to convert existing action items to extended format when accessed
- Default values ensure existing code continues to work
- UI components check for extended fields before displaying (optional features)

---

### 2. Agent Transition Logic & Edge Cases

**Issue:** Unclear when Marketing Agent transitions to Property Management Agent.

**Solution:** Define explicit transition rules and edge cases:

```typescript
interface AgentTransitionRules {
  // User Status Detection
  determineUserStatus(user: User, thread: Thread, conversationContext: ConversationContext): 'potential_tenant' | 'tenant' | 'prospect';
  
  // Transition Triggers
  triggers: {
    // Explicit transition signals
    leaseSigned: boolean; // Property Manager marks lease as signed in backend
    applicationApproved: boolean; // Application status changes to "approved" in backend
    moveInDateSet: boolean; // Move-in date confirmed in conversation or backend
    
    // Conversation-based signals
    firstRentPayment: boolean; // First rent payment mentioned or confirmed
    moveInConfirmed: boolean; // Move-in completed (keys given, property accessed)
    
    // Manual override
    manuallySetStatus: 'potential_tenant' | 'tenant'; // Property Manager explicitly sets status
  };
  
  // Transition Logic
  transitionLogic: {
    // When status changes from potential_tenant → tenant
    onStatusChange: (oldStatus: string, newStatus: string) => {
      if (oldStatus === 'potential_tenant' && newStatus === 'tenant') {
        // Switch from Marketing Agent to Property Management Agent
        // Preserve conversation context
        // Notify AI Agent of transition (for context preservation)
      }
    };
    
    // Edge Case: User is both prospect AND existing tenant
    handleDualStatus: (userId: string, properties: Property[]) => {
      // User can be:
      // - Tenant at Property A (existing lease)
      // - Prospect at Property B (shopping for new property)
      // Solution: Thread is linked to specific property, agent selected per thread
    };
  };
  
  // Agent Selection Per Thread
  selectAgent(thread: Thread): 'marketing' | 'property_management' {
    // Check thread's property association
    // Check user status for that specific property
    // Check conversation context (inquiry vs. existing tenant communication)
    
    if (thread.propertyId && thread.userStatus === 'tenant') {
      return 'property_management';
    }
    
    if (thread.userStatus === 'potential_tenant' || thread.userStatus === 'prospect') {
      return 'marketing';
    }
    
    // Default: marketing (for new inquiries)
    return 'marketing';
  };
}
```
...
**Edge Cases Handled:**
- **User shopping for multiple properties:** Each thread has independent agent based on property-specific status
- **Status transition mid-conversation:** AI Agent reads full context, preserves conversation history
- **Manual status override:** Property Manager can manually set user status, triggers agent switch
- **Lease renewal conversations:** Property Management Agent handles (user is existing tenant)

---

### 3. Channel Routing Edge Cases & Failure Modes

**Issue:** Channel selection logic doesn't handle failures, simultaneous channels, or delivery issues.

**Solution:** Add comprehensive failure handling:

```typescript
interface ChannelRoutingLogic {
  // Enhanced Channel Selection with Failure Handling
  selectChannel(thread: Thread, messageType: string, messageContent: string): ChannelSelection {
    const primaryChannel = determinePrimaryChannel(thread, messageType);
    const fallbackChannels = determineFallbackChannels(primaryChannel, messageType);
    
    return {
      primary: primaryChannel,
      fallbacks: fallbackChannels,
      retryPolicy: getRetryPolicy(primaryChannel),
    };
  };
  
  // Failure Handling
  handleDeliveryFailure(message: Message, failedChannel: Channel): Promise<ChannelResult> {
    // 1. Check if message has fallback channels
    if (message.fallbackChannels && message.fallbackChannels.length > 0) {
      // Try next fallback channel
      const nextChannel = message.fallbackChannels[0];
      return attemptDelivery(message, nextChannel);
    }
    
    // 2. If no fallback, use channel priority logic
    const alternativeChannel = selectAlternativeChannel(failedChannel, message.thread);
    if (alternativeChannel) {
      return attemptDelivery(message, alternativeChannel);
    }
    
    // 3. If all channels fail, mark message as failed and notify user
    return markAsFailed(message, 'All delivery channels failed');
  };
  
  // Simultaneous Channel Handling
  handleSimultaneousChannels(thread: Thread, incomingMessage: IncomingMessage): ThreadRouting {
    // Prospect uses multiple channels (e.g., SMS + Messenger)
    // Solution: Route all messages to same thread based on identity linking
    
    // 1. Check identity linking table
    const linkedIdentities = getLinkedIdentities(incomingMessage.senderIdentifier);
    
    // 2. Find thread with matching participant
    const existingThread = findThreadByParticipant(linkedIdentities.userId);
    
    // 3. If found, route to existing thread
    // If not found, create new thread and link identity
    
    return {
      threadId: existingThread?.id || createNewThread(linkedIdentities),
      channel: incomingMessage.channel,
      mergeWithExisting: !!existingThread,
    };
  };
  ...
  channelFailureModes: {
    sms: {
      // Twilio-specific failures
      invalidNumber: 'Format phone number and retry';
      carrierBlocked: 'Try alternative channel (Email)';
      rateLimit: 'Queue message, retry with exponential backoff';
      accountSuspended: 'Alert user, disable SMS channel';
    };
    messenger: {
      // Facebook Messenger-specific failures
      pageNotConnected: 'Alert user to reconnect Facebook Page';
      messageBlocked: 'Try alternative channel (SMS)';
      apiError: 'Retry with exponential backoff';
    };
    email: {
      // Email-specific failures
      invalidEmail: 'Validate email format, alert user';
      bounce: 'Mark email as invalid, try SMS';
      spamFilter: 'Alert user, suggest alternative channel';
      mailboxFull: 'Try alternative channel (SMS)';
    };
  };
  ...
  retryPolicies: {
    sms: {
      ...
      nonRetryableErrors: ['invalid_number', 'account_suspended'],
    };
    messenger: {
      maxRetries: 2,
      backoffStrategy: 'exponential', // 2s, 4s
      retryableErrors: ['api_error', 'temporary_failure'],
      nonRetryableErrors: ['page_not_connected', 'message_blocked'],
    };
    email: {
      maxRetries: 2,
      backoffStrategy: 'linear', // 5min, 10min
      retryableErrors: ['temporary_failure', 'server_error'],
      nonRetryableErrors: ['invalid_email', 'bounce', 'spam_filter'],
    };
  };
}
```
...
### 4. Backend Connector Error Handling & Resilience

**Issue:** Backend connector interface doesn't specify error handling, retry logic, or partial data scenarios.

**Solution:** Add comprehensive error handling specification:
...
interface BackendConnector {
  ...
  retryPolicy: {
    ...
    nonRetryableErrors: string[]; // Error codes that should fail immediately
  };
  ...
  rateLimiting: {
    ...
    handleRateLimit: (error: RateLimitError) => Promise<void>; // Wait and retry
  };
  
  // Partial Data Handling
  handlePartialData: (partialData: Partial<Property>, error: BackendError) => {
    // Return partial data with warning flags
    // AI Agent can work with partial data and indicate uncertainty
    return {
      data: partialData,
      completeness: 'partial',
      missingFields: ['rent', 'utilities'],
      warning: 'Some property data unavailable',
    };
  };
  
  // Authentication Failure Handling
  handleAuthFailure: (error: AuthError) => {
    // 1. Attempt token refresh if refresh token available
    // 2. If refresh fails, notify user to re-authenticate
    // 3. Cache failed requests for retry after re-auth
    // 4. Disable AI Agent features that require backend until re-auth
  };
  ...
  timeout: {
    ...
    perMethodTimeout: {
      getProperty: 3000,
      getAvailability: 10000, // Calendar queries can be slower
      createAppointment: 5000,
    };
    handleTimeout: (method: string, timeout: number) => {
      // Return cached data if available
      // Otherwise, return error with suggestion to retry
      return {
        error: 'timeout',
        message: `${method} timed out after ${timeout}ms`,
        suggestion: 'retry',
        cachedDataAvailable: checkCache(method),
      };
    };
  };
  ...
  circuitBreaker: {
    ...
    // When circuit is open, fail fast without calling backend
    handleCircuitOpen: () => {
      return {
...
class PropertyManagementConnector implements BackendConnector {
  ...
  async getProperty(propertyId: string): Promise<Property> {
    ...
    try {
      ...
      const property = this.transformProperty(response.data);
      
      // Cache successful response
      await this.cacheProperty(propertyId, property);
      ...
      const cached = await this.getCachedProperty(propertyId);
      if (cached) {
        return { ...cached, fromCache: true, warning: 'Using cached data' };
      }
...
### 5. Property Auto-Assignment Failure Modes & Timeouts

**Issue:** Property detection logic doesn't specify timeout, manual override process, or failure handling.

**Solution:** Add comprehensive failure handling:

```typescript
interface PropertyAutoAssignment {
  // Detection Logic with Timeout
  detectProperty(message: IncomingMessage, context: ConversationContext): Promise<PropertyDetectionResult> {
    const timeout = 5000; // 5 second timeout for AI detection
    
    return Promise.race([
      // AI Detection
      this.aiDetectProperty(message, context),
      
      // Timeout handler
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Property detection timeout')), timeout)
      ),
    ]).catch(error => {
      // On timeout or failure, return manual assignment required
      return {
        success: false,
        requiresManualAssignment: true,
        suggestions: this.generatePropertySuggestions(context),
        timeout: true,
      };
    });
  };
  ...
  failureModes: {
    ...
    noPropertyDetected: {
      action: 'prompt_user_for_selection',
      ui: {
        message: 'Which property is this conversation about?',
        suggestions: this.getAllProperties(), // Show all properties for selection
        allowCreateNew: true, // Allow creating new property if not found
      },
    };
    
    // Multiple properties match (ambiguous)
    ambiguousProperty: {
      action: 'show_disambiguation_ui',
      ui: {
        message: 'I found multiple matching properties. Which one?',
        options: this.getMatchingProperties(), // Show top 3-5 matches
        allowSearch: true,
      },
    };
    ...
    detectionTimeout: {
      ...
      ui: {
        message: 'Property detection took too long. Please select manually:',
        suggestions: this.getRecentProperties(), // Show recent properties
        allowSearch: true,
      },
    };
    ...
    aiDetectionFailed: {
      ...
      fallback: {
        ...
        searchProperties: this.searchPropertiesByKeywords(keywords),
      },
    };
  };
  ...
  manualOverride: {
    // Property Manager can manually assign property to thread
    assignProperty: (threadId: string, propertyId: string) => {
      // 1. Link thread to property
      // 2. Update thread metadata
      // 3. Notify AI Agent of property context
      // 4. Update property's thread list
    };
    
    // Property Manager can change property assignment
    reassignProperty: (threadId: string, oldPropertyId: string, newPropertyId: string) => {
      // 1. Remove thread from old property
      // 2. Assign to new property
      // 3. Update AI Agent context
      // 4. Log reassignment for audit
    };
    
    // Property Manager can unassign property
    unassignProperty: (threadId: string, propertyId: string) => {
      // 1. Remove property link from thread
      // 2. Thread becomes "unassigned"
      // 3. AI Agent works without property context
      // 4. Property Manager can reassign later
    };
  };
  
  // Property Suggestion Algorithm
  generatePropertySuggestions(context: ConversationContext): Property[] {
    // 1. Check recent properties (last 30 days)
    // 2. Check properties with similar addresses mentioned
    // 3. Check properties with active inquiries
    // 4. Check user's most accessed properties
    // 5. Return top 5 suggestions
    
    return this.rankProperties([
      ...this.getRecentProperties(30),
      ...this.searchByAddress(context.mentionedAddresses),
      ...this.getPropertiesWithActiveInquiries(),
      ...this.getUserMostAccessedProperties(),
    ]).slice(0, 5);
  };
}
```
...
### 6. Export Requirements for Large Threads & Media

**Issue:** Export section doesn't specify handling for large threads (10,000+ messages) or media files.

**Solution:** Add comprehensive export requirements:

```typescript
interface ExportRequirements {
  // Large Thread Handling
  largeThreadStrategy: {
    // Pagination for PDF export
    maxMessagesPerPage: 100; // Default: 100 messages per PDF page
    
    // Chunking for very large threads
    chunkSize: 1000; // Export in chunks of 1000 messages
    
    // Progress Indication
    showProgress: true; // Show export progress for large threads
    estimatedTime: calculateEstimatedTime(messageCount); // Estimate export time
    
    // Streaming Export (for very large threads)
    streaming: {
      enabled: true; // For threads > 5000 messages
      format: 'pdf_chunked', // Export as multiple PDF files
      chunkNaming: 'thread_export_part_1.pdf', 'thread_export_part_2.pdf',
    };
  };
  ...
  mediaHandling: {
    // Include in PDF
    includeMediaInPDF: {
      images: true; // Embed images in PDF
      videos: false; // Videos: Include link/thumbnail only (PDF limitation)
      files: false; // Files: Include link/thumbnail only
    };
    
    // Separate Media Export
    exportMediaSeparately: {
      enabled: true; // For threads with many media files
      format: 'zip', // Zip file containing all media
      structure: {
        'images/': 'All images organized by date',
        'videos/': 'All videos organized by date',
        'files/': 'All files organized by date',
      },
    };
    
    // Media Size Limits
    sizeLimits: {
      maxIndividualFileSize: 50 * 1024 * 1024; // 50MB per file
      maxTotalExportSize: 500 * 1024 * 1024; // 500MB total export
      handleOversizedMedia: 'skip_with_warning', // Skip files exceeding limit
    };
  };
  
  // Export Performance
  performance: {
    // Timeout for export generation
    exportTimeout: 300000; // 5 minutes max export time
    
    // Background Export
    backgroundExport: {
      enabled: true; // For exports > 1000 messages
      notification: 'Export will be ready shortly. You will be notified when complete.',
      storage: 'cloud_storage', // Store export in cloud, provide download link
    ...
    progressTracking: {
      ...
      stages: ['preparing', 'processing_messages', 'generating_pdf', 'finalizing'],
    };
  };
  ...
  completeness: {
    ...
    verifyCompleteness: true; // Check that all messages are in export
    
    // Missing Messages Handling
    handleMissingMessages: {
      action: 'warn_user', // Warn if messages are missing
      reason: 'Message may have been deleted or inaccessible',
      includeMetadata: true, // Include metadata about missing messages
    };
    
    // Message Ordering
    messageOrdering: 'chronological', // Ensure chronological order
    
    // Duplicate Detection
    detectDuplicates: true; // Detect and handle duplicate messages
  };
  
  // Export Formats
  formats: {
    pdf: {
      // Primary format for legal compliance
      includeMetadata: true;
      includeHeaders: true;
      includeFooters: true;
      pageNumbers: true;
      tableOfContents: false; // For very large exports, consider TOC
    };
...
### 7. Implementation Timeline with Dependencies & Critical Path

**Issue:** Implementation phases don't specify dependencies or identify critical path items.

**Solution:** Add dependency analysis and critical path:
...
interface ImplementationTimeline {
  phases: {
    '3.1': {
      ...
      criticalPath: true; // Blocks all subsequent phases
      ...
      parallelWork: [
        ...
        'Identity linking schema design',
      ];
    };
    ...
    '3.2': {
      ...
      duration: 'Weeks 5-8';
      dependencies: ['3.1']; // Requires foundation to be complete
      criticalPath: true;
      ...
      parallelWork: [
        // Can integrate channels in parallel
        'Messenger integration',
        'Email integration',
        'Roomies.com integration (if API available)',
      ];
    };
    ...
    '3.3': {
      ...
      duration: 'Weeks 9-12';
      dependencies: ['3.1', '3.2']; // Requires channels to be working
      criticalPath: true;
    ...
    '3.4': {
      ...
      dependencies: ['3.1']; // Requires foundation, but can start in parallel with 3.2/3.3
      criticalPath: false; // Can be done in parallel with 3.3
    ...
    '3.5': {
      ...
      dependencies: ['3.1', '3.2', '3.3', '3.4']; // Requires core features
      criticalPath: false; // Can be done in parallel with 3.6
    ...
    '3.6': {
      ...
      dependencies: ['3.3']; // Requires AI agent to be working
      criticalPath: false; // Can be done in parallel with 3.5
    ...
    '3.7': {
      ...
      dependencies: ['3.1', '3.2', '3.3', '3.4']; // Requires core features
      criticalPath: false; // Final polish phase
  ...
  criticalPath: [
    '3.1 Foundation', // Must complete first
    '3.2 Multi-Channel Expansion', // Blocks 3.3
    '3.3 AI Agent Participation', // Core feature, blocks 3.5 and 3.6
    // 3.4, 3.5, 3.6 can be done in parallel after 3.3
    // 3.7 is final polish
  ];
  
  // Parallel Work Opportunities
  parallelWork: {
    during_3_2: ['3.4 Organization System (early work)'], // Can start UI work
    during_3_3: ['3.4 Organization System', '3.5 Web Embeddable (early work)'], // Can work in parallel
    during_3_5: ['3.6 Backend Connector'], // Can work in parallel
  };
  
  // Risk Mitigation
  riskMitigation: {
    // If 3.2 delays, impact on 3.3
    '3.2_delay': {
      impact: '3.3 cannot start',
      mitigation: 'Start 3.4 early to maintain progress',
    };
    
    // If 3.3 delays, impact on 3.5 and 3.6
    '3.3_delay': {
      impact: '3.5 and 3.6 blocked',
      mitigation: 'Focus on 3.4 completion, prepare 3.5/3.6 design work',
    };
    
    // Backend connector complexity
    '3.6_complexity': {
      impact: 'May take longer than estimated',
      mitigation: 'Start with simple REST connector, add complexity incrementally',
    };
  };
}
```
...
**Document Status:** ✅ Refined - Execution-Ready with Critical Refinements  
**Next Action:** Review refinements → Merge into PRD → Begin Phase 3 Implementation Planning
