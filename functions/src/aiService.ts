...
export class AIService {
  ...
  async generateThreadSummary(
    ...
    try {
      ...
      const limitedMessages = messages.slice(-this.config.features.threadSummary.maxMessages);
    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an AI assistant that performs semantic search on conversations. Return relevant messages with relevance scores.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async getProactiveSuggestions(
    ...
    try {
      ...
      const response = await chatModel.invoke([
        new SystemMessage('You are a proactive AI assistant that monitors conversations and suggests helpful actions. Provide contextual suggestions based on conversation patterns and user needs.'),
  ...
  private buildSummaryPrompt(messages: any[]): string {
    const messageTexts = messages.map((msg, index) => 
      `${index + 1}. ${msg.text} (${msg.timestamp})`
    ).join('\n');
...
Provide 3-5 specific, actionable suggestions with confidence scores.`;
  ...
  private parseSummaryResponse(content: string): AIFeatures.ThreadSummary {
    ...
    const summary = lines.find(line => line.toLowerCase().includes('summary')) || content;
    
    return {
      id: `summary_${Date.now()}`,
      threadId: '',
      summary: summary,
      keyPoints: lines.filter(line => line.startsWith('•') || line.startsWith('-')),
      actionItems: [],
  ...
  private parseActionItemsResponse(content: string): AIFeatures.ActionItem[] {
    ...
    lines.forEach((line, index) => {
      if (line.trim() && (line.includes('action') || line.includes('task') || line.includes('todo'))) {
        actionItems.push({
  ...
  private parseProactiveResponse(content: string): AIFeatures.ProactiveSuggestion[] {
    const lines = content.split('\n');
