...
interface AIActions {
  ...
  updateActionItem: (
    threadId: string,
    actionItemId: string,
    updates: Partial<AIFeatures.ActionItem>
  ) => void;
  ...
  setMessagePriority: (
    messageId: string,
    priority: AIFeatures.PriorityLevel
  ) => void;
  getMessagePriority: (
    messageId: string
  ) => AIFeatures.PriorityLevel | undefined;
  ...
  setProactiveSuggestions: (
    threadId: string,
    suggestions: AIFeatures.ProactiveSuggestion[]
  ) => void;
...
export const useAIStore = create<AIState & AIActions>((set, get) => ({
  ...
  getProactiveSuggestions: threadId =>
    get().proactiveSuggestions[threadId] || [],
  ...
  addProactiveSuggestion: (threadId, suggestion) =>
    set(state => ({
      proactiveSuggestions: {
        ...state.proactiveSuggestions,
        [threadId]: [
          ...(state.proactiveSuggestions[threadId] || []),
          suggestion,
        ],
      },
    })),
