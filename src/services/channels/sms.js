...
const twilioConfig_1 = require("./twilioConfig");
const twilioStatusMapper_1 = require("./twilioStatusMapper");
/**
 * Twilio SMS Adapter
 *
...
class TwilioSMSAdapter {
    ...
    constructor(credentials) {
        ...
        this.twilioClient = (0, twilioConfig_1.getTwilioClient)(credentials);
    }
    /**
    ...
    async send(message) {
        ...
        try {
            ...
            const fromPhoneNumber = message.senderIdentifier || this.twilioPhoneNumber;
            ...
            if ((_a = message.metadata) === null || _a === void 0 ? void 0 : _a.channelSpecific) {
            ...
            const twilioResult = await this.twilioClient.messages.create(twilioMessage);
            // Convert Twilio response to ChannelMessageResult
            return {
                channelMessageId: twilioResult.sid,
                status: (0, twilioStatusMapper_1.mapTwilioStatusToUnifiedStatus)(twilioResult.status),
                timestamp: new Date(twilioResult.dateCreated
    ...
    receive(webhookPayload) {
        ...
        const unifiedMessage = {
            ...
            status: (0, twilioStatusMapper_1.mapTwilioStatusToUnifiedStatus)(webhookPayload.MessageStatus || 'delivered'),
            metadata: {
                channelSpecific: {
    ...
    async getStatus(messageId) {
        try {
            ...
            return (0, twilioStatusMapper_1.mapTwilioStatusToUnifiedStatus)(message.status);
        }
        catch (error) {
     ...
     * @returns Date object
     */
    parseTwilioTimestamp(timestamp) {
        if (!timestamp) {
            return new Date();
        }
    ...
    validatePhoneNumber(phoneNumber) {
        ...
        if (!e164Pattern.test(phoneNumber)) {
            throw new Error(`Invalid phone number format. Expected E.164 format (e.g., +15551234567), got: ${phoneNumber}`);
        }
    }
    /**
