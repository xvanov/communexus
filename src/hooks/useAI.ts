...
export const useAI = () => {
  ...
  const extractActionItems = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to extract action items';
        setError(errorMessage);
        return [];
      } finally {
        setLoading(false);
  ...
  const detectPriority = useCallback(
    async (
      messageId: string,
      message: any
  ...
  const smartSearch = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to perform smart search';
        setError(errorMessage);
        return [];
      } finally {
        setLoading(false);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to get proactive suggestions';
        setError(errorMessage);
        return [];
      } finally {
        setLoading(false);
